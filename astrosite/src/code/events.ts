...
export function getAllEventsGroupedByChronology(providedFilter?: (e: Event) => boolean): {
    ...
    getAllEvents(providedFilter).forEach((event) => {
        ...
        if (isFuture) {
            ...
            console.warn(
                `How is this possible? An event is neither past, ongoing, nor upcoming: ${JSON.stringify(event, null, 2)}`,
            );
        }
    });
    ...
    return {
        ongoing: ongoingEvents.sort(sortByDate),
        upcoming: upcomingEvents.sort(sortByDate),
        past: pastEvents.sort(sortByDate),
    };
}
...
const populateMissingParticipants = (event: Event): Event => {
    ...
    if (event.participants.length === 0 && event.results) {
        if (event.format === EventFormat.Matchplay) {
            ...
            console.warn(
                `Don't know how to populate an empty participants list for event ${JSON.stringify(event, null, 2)}`,
            );
        }
    }
    return event;
...
const populateUpdatedHandicaps = (event: Event): Event => {
    if (isHectorEvent(event) && !isPastEvent(event)) {
        function updateHcp(id: string, handicap: number | undefined) {
            return { id, handicap: getPlayerHandicapById(id) ?? handicap };
        }
...
export function getEventById(id: string): Event | undefined {
    ...
    try {
        ...
        console.error(
            `getEventById(${JSON.stringify(id)}) failed to parse event data: ${JSON.stringify(_event, null, 2)}`,
            err,
        );
        return undefined;
    }
...
export function getEventsAtCourse(courseId: string): Array<Event> {
    return eventsData
        .filter((event) => (event as HectorEvent)?.courses?.includes(courseId))
