    ...
    const playerListPromises = players.map((player: any) => {
        ...
        if (playerObject && !playerObject.club) {
            ...
            for (const source of handicapSources) {
                const foundInClubs: Promise<string[]> = source
                    .resolveClubMembership(playerObject.name.first, playerObject.name.last)
                    .then((clubs) => clubs.map((club) => club.name));
                foundInClubs.then((clubs) =>
                    console.log(
                        `${getPlayerName(playerObject)} found at ${clubs.length} clubs via ${
                            source.name
            ...
            return Promise.all(promises)
                ...
                .then((club) => {
                    if (club) {
                        ...
                        console.log(
                            `Club found for ${playerObject.name.first} ${playerObject.name.last}: ${playerObject.club}`,
                        );
                    }
