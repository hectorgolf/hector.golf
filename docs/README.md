...
| [`courses-in-the-admin.md`](plans/courses-in-the-admin.md) | The seventeen golf courses: into Firestore, rendered, and editable | **Steps 1-4,** on 2026-09-21, bar the flag. Tee names normalised, courses seeded as a mirror with a synthetic id per tee, read-only pages, and an editor for identity, contact, prose and tees — renaming a tee moves the scorecard's keys with it. `COURSES_ARE_OWNED` is still false; flipping it is the whole of what is left |
