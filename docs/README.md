...
| | | executed? |
| --- | --- | --- |
| [`authoring-players-and-events.md`](plans/authoring-players-and-events.md) | Moving players and Hector events from the mirrored column to the owned one, and giving each an editor — the one piece of work the other two are waiting on | **Step 0,** on 2026-09-20, plus read-only pages for both. Every ownership guard now asks `ownership.ts` rather than naming matchplay, and `PLAYERS_ARE_OWNED` is the one edit that flips the players half. No ownership has moved: the admin still authors matchplay and nothing else, because step 1 has to move two scheduled writers first. Finnkampen was the pilot and is now out of scope |
