...
| `player.biography` | Authored | Human | Generated by Gemini, then owned by whoever edits it. The generator rewrites every biography it is not told to leave alone, so an unlocked edit survives a fortnight at most; `player.biographyLocked` is how you tell it. See below |
| `player.biographyLocked` | Authored | Human | Unset everywhere, and unset means "this paragraph is still the generator's"; set, it stops the rewrite and the run says whose text it left alone. See below |
...
| `player.biographyLocked` | **Built.** Stops `update-player-biographies` rewriting one player's text. The run logs who it left alone and why, and still shows the model the locked biographies so a regenerated one does not echo a published sentence. What is left of [`plans/biography-locking.md`](../plans/biography-locking.md) is the admin UI for setting it |
