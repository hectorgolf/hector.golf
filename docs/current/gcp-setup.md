...
| Six Secret Manager secrets | `github-dispatch-token`, `wisegolf-username` and `wisegolf-password` for the admin, and `gemini-api-key`, `astrosite-api-key` and `hector-app-api-key` for the functions. The admin reads `astrosite-api-key` too, since 2026-09-20, to call `GeneratePlayerBiography`. Terraform creates every container and never a value — those go in by hand with `gcloud secrets versions add` |
