...
| `terraform apply` fails with `terraform-ci@… does not have storage.buckets.create access` | The bucket in [`storage.tf`](../../terraform/storage.tf) is created by the same apply that grants this identity `roles/storage.admin`, and Terraform builds unrelated resources in parallel. It is ordered with `depends_on` since 2026-09-21, so this should not recur — but ordering does not make an IAM grant *propagate*, and a fresh project can still lose that race once. Re-run the apply; the second one succeeds. This is the same propagation delay the `iap.webServices.getSettings` row below describes |
