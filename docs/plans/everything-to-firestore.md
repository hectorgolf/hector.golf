...
the same day; `handicaps-to-firestore.md` was that plan, and it was deleted when it finished — see
[`docs/README.md`](../README.md) for where its content went, and
[`architecture.md`](../current/architecture.md) §8 for what survives of it. This document is kept as
a worked alternative rather than as a proposal: the code is real and runs, which is what makes the
comparison at the end worth anything. Read [Which one](#which-one) for what the choice actually
turned on.*

## Where the implementation is

**Moved onto `main` on 2026-09-21, and the branch deleted.** This document used to live on
`claude/everything-to-firestore` with the code it describes, and said the branch was kept for that
reason. Keeping a branch alive indefinitely to hold a document is the wrong half of that argument:
the document is what gets read, and it was invisible to anybody who did not already know the branch
existed.

The code is not lost. GitHub keeps a pull request's refs permanently, so the implementation is still
fetchable even though the branch is gone:

```bash
git fetch origin refs/pull/145/head:everything-to-firestore
git log --oneline main..everything-to-firestore
```

That is [PR #145](https://github.com/hectorgolf/hector.golf/pull/145), tip
`d4fceb19a5ea3321083ab3368c5c47c4cd58df7a`, six commits on top of `main` as it stood on 2026-09-20.
Deleting the branch closed that PR; its diff and its commits stay readable.

So the comparison below still has its evidence behind it — one command further away than it was, and
in a place that does not depend on somebody remembering not to tidy a branch up.
